...
async function getAllGrades() {
  const res = await axious.get(API_URL);
...
async function insertGrade(grade) {
  const res = await axious.post(API_URL, grade);
  return res.data.id;
}

async function updateGrade(grade) {
  const res = await axious.put(API_URL, grade);
  return res.data;
}

async function deleteGrade(grade) {
  const res = await axious.delete(`${API_URL}/${grade.id}`);
  return res.data;
}
